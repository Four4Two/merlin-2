...
func CreateUpgradeHandler(
	...
	return func(ctx sdk.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
		...
		for _, assetPair := range assetPairs {
			...
			oneFury := sdk.NewCoin(QuoteAsset, sdk.NewInt(1000000))
			linkedClassicPool, err := keepers.PoolManagerKeeper.GetPool(ctx, assetPair.LinkedClassicPool)
			if err != nil {
				return nil, err
			}
			respectiveBaseAsset, err := keepers.GAMMKeeper.CalcOutAmtGivenIn(ctx, linkedClassicPool, oneFury, assetPair.BaseAsset, sdk.ZeroDec())
			if err != nil {
				return nil, err
			}

			// Create a full range position via the community pool with the funds we calculated above.
			fullRangeCoins := sdk.NewCoins(respectiveBaseAsset, oneFury)
