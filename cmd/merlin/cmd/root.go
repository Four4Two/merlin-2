...
var (
	//go:embed "merlin-1-assetlist.json" "fury-test-5-assetlist.json"
	assetFS   embed.FS
	mainnetId = "merlin-1"
...
func loadAssetList(initClientCtx client.Context, cmd *cobra.Command, basedenomToIBC, IBCtoBasedenom bool) (map[string]DenomUnitMap, map[string]string) {
	...
	if chainId == mainnetId || chainId == "" {
		fileName = "cmd/merlin/cmd/merlin-1-assetlist-manual.json"
	} else if chainId == testnetId {
		fileName = "cmd/merlin/cmd/fury-test-5-assetlist-manual.json"
	} else {
		return nil, nil
	}
	...
	if err != nil {
		...
		if chainId == mainnetId || chainId == "" {
			fileName = "merlin-1-assetlist.json"
		} else if chainId == testnetId {
...
func initRootCmd(rootCmd *cobra.Command, encodingConfig params.EncodingConfig) {
	...
	rootCmd.AddCommand(
		...
		PrintEnvironmentCmd(),
		UpdateAssetListCmd(merlin.DefaultNodeHome, merlin.ModuleBasics),
	)
...
Outputs:
	- cmd/merlin/cmd/merlin-1-assetlist-manual.json for merlin-1
	- cmd/merlin/cmd/fury-test-5-assetlist-manual.json for fury-test-5
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if args[0] == mainnetId || args[0] == "" {
				assetListURL = "https://raw.githubusercontent.com/four4two/assetlists/main/merlin-1/merlin-1.assetlist.json"
				fileName = "cmd/merlin/cmd/merlin-1-assetlist-manual.json"
			} else if args[0] == testnetId {
				assetListURL = "https://raw.githubusercontent.com/four4two/assetlists/main/fury-test-5/fury-test-5.assetlist.json"
				fileName = "cmd/merlin/cmd/fury-test-5-assetlist-manual.json"
			} else {
				return nil
			}
...
# bash example
echo '. <(merlin enable-cli-autocomplete bash)' >> ~/.profile
source ~/.profile

# zsh example
echo '. <(merlin enable-cli-autocomplete zsh)' >> ~/.zshrc
source ~/.zshrc
`,
