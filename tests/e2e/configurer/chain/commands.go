...
import (
	...
	app "github.com/four4two/merlin/v17/app"
...
func (n *NodeConfig) CreateBalancerPool(poolFile, from string) uint64 {
	...
	cmd := []string{"merlin", "tx", "gamm", "create-pool", fmt.Sprintf("--pool-file=/merlin/%s", poolFile), fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000ufury"}
...
func (n *NodeConfig) CreateStableswapPool(poolFile, from string) uint64 {
	...
	cmd := []string{"merlin", "tx", "gamm", "create-pool", fmt.Sprintf("--pool-file=/merlin/%s", poolFile), "--pool-type=stableswap", fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000ufury"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SubmitSuperfluidProposal(asset string, initialDeposit sdk.Coin) int {
	...
	cmd := []string{"merlin", "tx", "gov", "submit-proposal", "set-superfluid-assets-proposal", fmt.Sprintf("--superfluid-assets=%s", asset), "--title=\"superfluid asset prop\"", fmt.Sprintf("--description=\"%s superfluid asset\"", asset), "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit), "--gas=700000", "--fees=5000ufury"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) AddToExistingLock(tokens sdk.Int, denom, duration, from string, lockID int) {
	...
	path := fmt.Sprintf("/merlin/lockup/v1beta1/locked_by_id/%d", lockID)
...
func (n *NodeConfig) FundCommunityPool(sendAddress string, funds string) {
	...
	cmd := []string{"merlin", "tx", "distribution", "fund-community-pool", funds, fmt.Sprintf("--from=%s", sendAddress), "--gas=600000", "--fees=1500ufury"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
