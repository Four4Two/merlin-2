...
# 2nd NB: you can change MERLIN_PRUNING=nothing to MERLIN_PRUNING=default OR you could also set the pruning settings manually with MERLIN_PRUNING=custom
...
export MERLIN_PRUNING=nothing
export MERLIN_DB_BACKEND=pebbledb
export MERLIN_P2P_MAX_NUM_OUTBOUND_PEERS=500
export MERLIN_P2P_MAX_NUM_INBOUND_PEERS=500
export MERLIN_P2P_SEEDS=$(curl -s https://raw.githubusercontent.com/cosmos/chain-registry/master/merlin/chain.json | jq -r '[foreach .peers.seeds[] as $item (""; "\($item.id)@\($item.address)")] | join(",")')
export MERLIN_P2P_LADDR=tcp://0.0.0.0:2001
...
wget -O ~/.merlin/config/genesis.json https://github.com/four4two/networks/raw/main/merlin-1/genesis.json
