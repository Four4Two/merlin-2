...
done < <(find . -name go.mod -exec grep -l "github.com/four4two/merlin/v16" {} \; | grep -v  "^./go.mod$" | sed 's|/go.mod||' | sed 's|^./||')

replace_paths() {
    file="${1}"
    sed -i "s/github.com\/four4two\/merlin\/v${version_to_replace}/github.com\/four4two\/merlin\/v${NEXT_MAJOR_VERSION}/g" ${file}
}
